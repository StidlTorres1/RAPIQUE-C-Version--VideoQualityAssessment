...
if __name__ == "__main__":
    ...
    log_filename = 'extraction_log.csv'
    with open(log_filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Video ID', 'Filename', 'Database', 'Result (Score)', 'Execution Time (s)'])
        ...
        for i in range(args.ith, len(dataset)): # range(args.ith, args.ith+1): # 
            ...
            writer.writerow([i, video_names[i], args.database, current_data['score'].item(), execution_time])
            print(f'Processed video {i}: {video_names[i]}, Score: {current_data["score"].item()}, Execution time: {execution_time:.2f}s')
            
            end = time.time()
            print('{} seconds'.format(end-start))
        print(max_len)

        # Log total execution time at the end of the file
    total_time = time.time() - ini
    with open(log_filename, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Total Execution Time', '-', '-', '-', total_time])
        print('All videos processed. Total time elapsed: {:.2f} seconds'.format(total_time))

print('Time elapsed: {} seconds'.format(time.time() - ini))