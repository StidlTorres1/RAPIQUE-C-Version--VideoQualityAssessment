...
def evaluate_bvqa_one_split(i, X, y, log_short):
  if not log_short:
    ...
    t_start = time.time()
# If log_short is False, prints a message indicating the repetition number of the retention 
# test and starts time of the test.
...
# Performs a cross-validated grid search on the training set using regression models, 
# taking into account the specified dimensions, and then scaling the training data using (MinMaxScaler)
  ...
  #If log_short is False, prints the metric values and execution time.
  #Returns the best parameters and training and testing metrics.
  
def main(args):
  df = pandas.read_csv(args.mos_file, skiprows=[], header=None)
  array = df.values
  dataset_name = args.dataset_name[:-3] if args.dataset_name.endswith('i9') else args.dataset_name
  if dataset_name == 'LIVE_VQC':
      y = array[1:,1]
  elif dataset_name == 'KONVID_1K': # for LIVE-VQC & KONVID_1k
      y = array[1:,1]
  elif dataset_name == 'YOUTUBE_UGC':
      y = array[1:,1]
  elif dataset_name == 'LIVE_qualcomm':
      y = array[1:,1]
  elif dataset_name == 'CVD_2014':
      y = array[1:,1]
  elif dataset_name == 'all_combined':
      y = array[1:, 1]
  y = np.array(list(y), dtype=float)

  feature_filepath = args.feature_file
  if feature_filepath.endswith('.mat'):
      X_mat = scipy.io.loadmat(args.feature_file)
      X = np.asarray(X_mat['feats_mat'], dtype=float)
 
  elif feature_filepath.endswith('.xml'):
      # Read XML file
      tree = ET.parse(args.feature_file)
      # find element 'FeatFrames' 
      root = tree.getroot()
      feats_mat_element = root.find('FeatFrames')
      # find 'data' in 'FeatFrames'
      data_element = feats_mat_element.find('data')
      # Convert data into list
      df = pandas.DataFrame([row.split() for row in data_element.text.strip().split('\n')])
      # Replace '-nan(ind)' with np.nan and convert to float
      X = df.replace('-nan(ind)', np.nan).astype(float).values
      
  else:
      raise ValueError("feature file is not supported")
  ...
  if args.color_only and args.dataset_name == 'YOUTUBE_UGC':
      ...
      # eliminate gray-scale videos from X and y if using only color videos

  ## preprocessing
  X[np.isinf(X)] = np.nan #replace inf values with NaN
  imp = SimpleImputer(missing_values=np.nan, strategy='mean').fit(X) #impute Nan values with mean strategy
  X = imp.transform(X)
  ...
  # Save all iterations metrics in csv file
  df = pandas.DataFrame(all_iterations, columns=['SRCC_train', 'KRCC_train', 'PLCC_train', 'RMSE_train',
                                                 'SRCC_test', 'KRCC_test', 'PLCC_test', 'RMSE_test'])
  df.to_csv(f"result\\all_iterations_{args.dataset_name}_CPP.csv", index=False)
  ...
  scipy.io.savemat(args.out_file, 
      mdict={'all_iterations': np.asarray(all_iterations,dtype=float)})
