...
with open(log_file, 'w', newline='') as file:
    ...
    writer.writerow(['Filename', 'Database', 'SROCC', 'PLCC', 'KROCC', 'RMSE', 'Execution Time'])
...
if __name__ == "__main__":
    ...
    if args.test_ratio > 0:
        ...
        # Log the results to CSV including total execution time
        execution_time = time.time() - start_time
        total_time = time.time() - ini
        with open(log_file, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([test_index, args.database, SROCC, PLCC, KROCC, RMSE, execution_time, total_time])

        print("Test results: SROCC={:.4f}, PLCC={:.4f}, KROCC={:.4f}, RMSE={:.4f}, Execution Time={:.2f}s"
              .format(SROCC, PLCC, KROCC, RMSE, execution_time))
        print('Total Execution Time: {:.2f}s'.format(total_time))
